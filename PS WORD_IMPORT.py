...
/* File uploader: hide Streamlit Material icon text that appears as "upload" */
div[data-testid="stFileUploader"] button [data-testid="stIconMaterial"],
div[data-testid="stFileUploader"] button span[class*="material"],
...
def clean_word_cell(cell):
    if cell is None:
        return ""
    return "\n".join(
        clean_word_text(p.text)
        for p in cell.paragraphs
        if clean_word_text(p.text)
    ).strip()


def first_non_empty(values):
    for value in values:
        value = clean_word_text(value)
        if value:
            return value
    return ""


def extract_word_metadata(doc):
    metadata = {"institution": "", "responsable": "", "response_date": ""}
    ...
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                txt = clean_word_cell(cell)
                if txt:
                    all_text.append(txt)

    for txt in all_text[:30]:
        lower_txt = txt.lower()
        if lower_txt.startswith("institution") and ":" in txt:
            metadata["institution"] = clean_word_text(txt.split(":", 1)[1])
        elif lower_txt.startswith("responsable") and ":" in txt:
            metadata["responsable"] = clean_word_text(txt.split(":", 1)[1])
        elif lower_txt.startswith("date") and ":" in txt:
            metadata["response_date"] = clean_word_text(txt.split(":", 1)[1])

    return metadata


def parse_word_template(uploaded_docx):
    doc = Document(uploaded_docx)
    tables = doc.tables

    if len(tables) < 27:
        raise ValueError(
            "Le fichier Word importé ne correspond pas au modèle attendu. "
            "Merci d'utiliser le modèle protégé officiel."
        )

    metadata_from_word = extract_word_metadata(doc)
    ...
    stakeholders_rows = []
    stakeholders_table = tables[0]
    for row in stakeholders_table.rows[1:]:
        cells = row.cells
        item = {
            "categorie": clean_word_cell(cells[0]) if len(cells) > 0 else "",
            "nom": clean_word_cell(cells[1]) if len(cells) > 1 else "",
            "poste": clean_word_cell(cells[2]) if len(cells) > 2 else "",
            "organisme_affiliation": clean_word_cell(cells[3]) if len(cells) > 3 else "",
        }
        if any(item.values()):
            stakeholders_rows.append(item)

    internal_analysis = {}
    for index, theme in enumerate(internal_themes, start=1):
        internal_analysis[theme] = clean_word_cell(tables[index].cell(0, 0))

    external_analysis = {}
    for offset, theme in enumerate(external_themes):
        external_analysis[theme] = clean_word_cell(tables[15 + offset].cell(0, 0))

    swot_internal = []
    swot_internal_table = tables[23]
    for row in swot_internal_table.rows[1:]:
        swot_internal.append({
            "Forces (Saisir une force par case)": clean_word_cell(row.cells[0]),
            "Faiblesses (Saisir une faiblesse par case)": clean_word_cell(row.cells[1]),
        })

    swot_external = []
    swot_external_table = tables[24]
    for row in swot_external_table.rows[1:]:
        swot_external.append({
            "Opportunités (Saisir une opportunité par case)": clean_word_cell(row.cells[0]),
            "Menaces (Saisir une menace par case)": clean_word_cell(row.cells[1]),
        })
    ...
    data = {
        "metadata": {},
        "introduction": {},
        "stakeholders": {"rows": stakeholders_rows},
